...
public class DBSupportH2Impl extends AbstractDBSupportImpl
{
  ...
  public DBSupportH2Impl()
  {
    ...
    System.setProperty(AktienDbServiceImpl.class.getName() + ".uppercase","true");
  ...
  {
    String password = AktienDBService.SETTINGS.getString("database.driver.h2.encryption.encryptedpassword",null);
    try
    {
      ...
      if (password == null)
      {
        ...
        AktienDBService.SETTINGS.setAttribute("database.driver.h2.encryption.encryptedpassword",Base64.encode(bos.toByteArray()));
  ...
  {
    String url = "jdbc:h2:" + Application.getPluginLoader().getPlugin(AktienPlugin.class).getResources().getWorkPath() + "/h2db/aktien";

    if (AktienDBService.SETTINGS.getBoolean("database.driver.h2.encryption",true))
      url += ";CIPHER=" + AktienDBService.SETTINGS.getString("database.driver.h2.encryption.algorithm","XTEA");
    if (AktienDBService.SETTINGS.getBoolean("database.driver.h2.recover",false))
    {
      Logger.warn("#############################################################");
    ...
    String addon = AktienDBService.SETTINGS.getString("database.driver.h2.parameters",null);
    if (addon != null)
      url += ";" + addon;
