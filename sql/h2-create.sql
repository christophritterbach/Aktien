...
CREATE VIEW v_aktien AS SELECT aktien.id, aktien.wkn, aktien.isin, aktien.bezeichnung
 , NVL(SUM(kauf.Anzahl), 0) AS anzahl, NVL(SUM(kauf.betrag), 0) AS betrag, NVL(SUM(kauf.kosten), 0) AS kosten, MIN(kauf.kauf_Datum) AS erster_kauf, MAX(kauf.kauf_Datum) AS letzter_kauf
